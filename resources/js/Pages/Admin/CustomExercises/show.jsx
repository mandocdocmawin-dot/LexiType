...
export default function ShowExercise({ exercise }) {
    ...
    return (
        <AdminLayout>
            <Head title={`${exercise.category} — Exercise`} />
                ...
                <div className="max-w-4xl mx-auto space-y-10">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 text-sm font-medium" style={{ color: '#64748b' }}>
                            ...
                            <span className="material-symbols-outlined text-sm">chevron_right</span>
                            <span style={{ color: '#bbc3ff' }}>{exercise.category}</span>
                        </div>
                        <Link href={route('admin.exercises.edit', exercise.id)}
                    ...
                    {/* Title */}
                    <div>
                        <span className="text-[10px] uppercase tracking-widest font-bold mb-2 block" style={{ color: 'rgba(99,102,241,0.8)' }}>Exercise Title</span>
                        <h1 className="text-5xl font-bold tracking-tight" style={{ fontFamily: 'Space Grotesk', color: '#dae2fd' }}>{exercise.category}</h1>
                    </div>

                    {/* Meta badges */}
                    <div className="flex items-center gap-4">
                        <span className="text-[10px] px-3 py-1.5 rounded-lg font-bold" style={{ background: 'rgba(78,222,163,0.1)', color: exercise.is_active ? '#4edea3' : '#94a3b8' }}>
                            {exercise.is_active ? 'PUBLISHED' : 'DRAFT'}
                        </span>
                        <span className="text-[10px] px-3 py-1.5 rounded-lg font-bold uppercase" style={{ background: '#020617', color: diffColor[exercise.difficulty_level] ?? '#94a3b8' }}>
                            {diffLabel[exercise.difficulty_level] ?? exercise.difficulty_level}
                        </span>
                        <span className="text-[10px] px-3 py-1.5 rounded-lg font-mono" style={{ background: '#020617', color: '#94a3b8' }}>
                            {charCount(exercise.content)} chars
                        </span>
                        <span className="text-xs" style={{ color: '#64748b' }}>Created {formatDate(exercise.created_at)}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-8" style={{ borderTop: '1px solid rgba(68,70,86,0.15)' }}>
                        ...
                        <Link href={route('admin.exercises.edit', exercise.id)}
                            ...
                            <span className="material-symbols-outlined text-lg">edit</span>Edit Exercise
                        </Link>
                    </div>
                </div>
            </div>
